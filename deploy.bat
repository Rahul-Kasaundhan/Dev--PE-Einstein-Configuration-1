...
echo.
echo Validating the first package using %TESTLEVEL% test level...
if %TESTLEVEL% == RunSpecifiedTests (
...
if %ERRORLEVEL% NEQ 0 (
    ...
    cls
    echo.
    set /p deploy1=Validation success. Do you want to deploy?[y/n] 
)

echo.
if %deploy1% == y (
    echo Deploying the first package using %TESTLEVEL% test level...
    if %TESTLEVEL% == RunSpecifiedTests (
...
echo.
echo Validating the second package using %TESTLEVEL% test level...
if %TESTLEVEL% == RunSpecifiedTests (
    call sfdx force:source:deploy -x manifest/package1.xml -u tempQuickDeploymentOrg -l %TESTLEVEL% -r %TESTCLASSES% -c
) else (
    call sfdx force:source:deploy -x manifest/package1.xml -u tempQuickDeploymentOrg -l %TESTLEVEL%  -c
)
...
if %ERRORLEVEL% NEQ 0 (
    ...
    cls
    echo.
    set /p deploy2=Validation success. Do you want to deploy?[y/n] 
)

echo.
if %deploy2% == y (
    echo Deploying the second package using %TESTLEVEL% test level...
    if %TESTLEVEL% == RunSpecifiedTests (
        call sfdx force:source:deploy -x manifest/package1.xml -u tempQuickDeploymentOrg -l %TESTLEVEL% -r %TESTCLASSES%
    ) else (
        call sfdx force:source:deploy -x manifest/package1.xml -u tempQuickDeploymentOrg -l %TESTLEVEL%
    )
)
